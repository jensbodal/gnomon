...
const cli = meow(intro.join("\n") + usage, {
  ...
  flags: {
    ...
    ignoreBlank: {
      aliases: ["ignore-blank", "quiet", "q", "i"],
      type: "boolean",
    },
    high: {
      type: "number",
...
if (flags.realTime && typeof flags.realTime !== "number") {
  flags.realTime = 500;
}

process.stdin.pipe(split()).pipe(gnomon(flags)).pipe(process.stdout);
